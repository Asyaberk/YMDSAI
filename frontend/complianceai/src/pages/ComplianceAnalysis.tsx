...
function DetailView({ docId, onBack }: DetailViewProps) {
  ...
  const scorable = detail.articles.length - kapsam;          // Kapsam Dışı excluded
  const total    = detail.articleCount || detail.articles.length;

  // Score: weighted average excluding Kapsam Dışı (same logic as backend)
  const expectedScore = scorable > 0
    ? Math.round((uyumlu * 100 + kismen * 60 + uyumsuz * 0) / scorable)
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Madde bazında YÖK uyum analizi · {new Date(detail.uploadDate).toLocaleDateString('tr-TR')}</p>
          </div>
        </div>
      ...
      <div className={`rounded-2xl border p-5 flex flex-col sm:flex-row sm:items-center gap-5
        ...
        <div className="relative w-20 h-20 shrink-0 mx-auto sm:mx-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[9px] text-slate-400 font-bold">UYUM</span>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-slate-500">Bu skor nasıl hesaplandı?</span>
          </div>
          <p className="text-xs text-slate-600 leading-relaxed mb-3">
            Belgenizin <strong>{total} maddesinden</strong> <strong>{scorable} tanesi</strong> YÖK mevzuatıyla karşılaştırıldı.
            {kapsam > 0 && <span className="text-slate-400"> ({kapsam} madde YÖK kapsamı dışında, skora dahil edilmedi.)</span>}
            {' '}Uyumlu maddeler <strong className="text-success">%100</strong>, kısmen uyumlu maddeler <strong className="text-warning">%60</strong>,
            uyumsuz maddeler <strong className="text-danger">%0</strong> puanlandı.
          </p>
          <div className="flex gap-4 flex-wrap">
            ...
            {kapsam > 0 && (
              <button onClick={() => setFilterStatus('Kapsam Dışı')}
                ...
                <MinusCircle size={13} /> {kapsam} Kapsam Dışı
              </button>
            )}
        ...
        {detail.complianceScore < 100 && (
          <div className="shrink-0 text-center px-5 py-4 bg-white rounded-xl border border-slate-100 shadow-sm">
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">%100'e kalan</p>
            <p className="text-2xl font-bold text-danger mt-1">{100 - detail.complianceScore}</p>
            <p className="text-[10px] text-slate-400 mt-0.5">{problemArticles.length} madde düzeltilmeli</p>
          </div>
        )}
      </div>

      {/* ── Acil Düzeltme Gerektiren Maddeler ── */}
      {problemArticles.length > 0 && (
        <div className="bg-danger/3 border border-danger/15 rounded-2xl p-5 space-y-3">
          <div className="flex items-center gap-2">
            <AlertTriangle size={16} className="text-danger" />
            <h3 className="text-sm font-bold text-slate-800">
              Düzeltilmesi Gereken {problemArticles.length} Madde
            </h3>
            <span className="text-[10px] text-slate-400 ml-auto">Tıklayarak detay görebilirsiniz</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(['Tümü', 'Uyumsuz', 'Kısmen Uyumlu', 'Uyumlu', 'Kapsam Dışı'] as const).map(f => {
              const cnt = f === 'Tümü' ? total : f === 'Uyumlu' ? uyumlu : f === 'Kısmen Uyumlu' ? kismen : f === 'Kapsam Dışı' ? kapsam : uyumsuz;
              return (
                <button key={f} onClick={() => setFilterStatus(f)}
          ...
          {filteredArticles.length === 0 ? (
            ...
            const simLabel = simPct >= 80 ? 'Yüksek örtüşme' : simPct >= 50 ? 'Orta örtüşme' : 'Düşük örtüşme';
                ...
                <div className="p-5 flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {/* Suggestion preview for non-compliant */}
                      ...
                      <div className="p-6 space-y-5 bg-slate-50/50">
                        ...
                        <div className={`p-4 rounded-xl border ${
                          ...
                          <p className={`text-[10px] font-bold uppercase tracking-widest mb-1 ${
                            ...
                            'text-danger'}`}>
                            {art.status === 'Uyumlu' ? '✅ Bu madde uyumlu' :
                             art.status === 'Kısmen Uyumlu' ? '⚠️ Kısmen uyumlu — revizyon önerilir' :
                             art.status === 'Kapsam Dışı' ? '⊖ Kapsam Dışı — YÖK bu konuyu düzenlemiyor' :
                             '❌ Uyumsuz — düzeltme gerekiyor'}
                          </p>
                          <p className="text-xs text-slate-600">
                            YÖK mevzuatıyla anlam örtüşmesi: <strong className={simColor}>%{simPct}</strong> —
                            {art.status === 'Kapsam Dışı' ? ' Bu madde üniversitenin kendi takdirine bırakılan bir alanı düzenliyor.' :
                             simPct >= 80 ? ' Madde büyük ölçüde YÖK gereklilikleriyle örtüşüyor.' :
                             simPct >= 50 ? ' Madde kısmen uyumlu; eksik veya çelişkili hükümler var.' :
                             ' Madde YÖK mevzuatıyla önemli ölçüde çelişiyor veya karşılanmıyor.'}
                          </p>
                        </div>

                        {art.text && (
                          <div>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Belgenizdeki Metin</p>
                            ...
                            <div className="flex items-center justify-between mb-2">
                              ...
                              {art.yokReference && (
                                <a href={yokRefToUrl(art.yokReference)} target="_blank" rel="noreferrer"
                                  ...
                                  Dayanak Mevzuata Git <ArrowUpRight size={10} />
                                </a>
                              )}
                            </div>
                        ...
                        {art.reasoning?.length > 0 && (
                          <div>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Değerlendirme Gerekçesi</p>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900 text-white p-6 rounded-[32px] space-y-5">
            <h3 className="text-base font-bold">Uyum Dağılımı</h3>
              ...
              return (
                <div key={s.label} className="space-y-1.5">
                  <div className="flex justify-between items-center text-xs">
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${s.text}`}>{s.count} madde</span>
            ...
            <button onClick={() => setViewerOpen(true)}
              ...
              <Eye size={16} /> Belgeyi PDF'te İncele
            </button>
          </div>
          ...
          <div className="space-y-3">
            <button onClick={() => navigate('/portal')}
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">Bu maddeler hakkında soru sor</p>
              </div>
