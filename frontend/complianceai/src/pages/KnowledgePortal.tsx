...
import { Search, Send, ThumbsUp, ThumbsDown, ExternalLink, Clock, User, Bot } from 'lucide-react';
...
export default function KnowledgePortal() {
  ...
  const handleSearch = (e?: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      setShowResult(true);
      setHistory(prev => [...prev, { q: query, a: 'YÖK 2547 sayılı kanun ve ilgili yönetmelikler uyarınca, sorduğunuz konu hakkında üniversite senatosunun belirlediği esaslar geçerlidir. Mevcut mevzuat, akademik özgürlük ve idari özerklik çerçevesinde bu durumu düzenlemektedir.' }]);
    }, 1500);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
        ...
        {history.map((chat, i) => (
          <div key={i} className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex justify-end">
              <div className="bg-slate-100 text-slate-700 px-4 py-2 rounded-2xl rounded-tr-none max-w-[80%] text-sm">
                {chat.q}
              </div>
            </div>
            <div className="flex gap-3">
              ...
              <div className="bg-white border border-gray-200 text-slate-700 px-4 py-3 rounded-2xl rounded-tl-none max-w-[80%] text-sm shadow-sm leading-relaxed">
                {chat.a}
              </div>
            </div>
          </div>
      ...
      {/* Search Input */}
      <div className="sticky bottom-8 bg-white/80 backdrop-blur-md p-4 rounded-3xl border border-gray-200 shadow-xl">
        <form onSubmit={handleSearch} className="relative">
          ...
          <button 
            type="submit"
            className="absolute right-2 top-2 bottom-2 bg-primary text-white px-6 rounded-xl font-medium text-sm flex items-center gap-2 hover:bg-opacity-90 transition-all"
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {suggestions.map((s) => (
            <button 
              key={s}
              className="text-[10px] bg-white border border-gray-200 text-slate-500 px-3 py-1.5 rounded-full hover:border-primary hover:text-primary transition-all"
              onClick={() => setQuery(s)}
            >
              {s}
            </button>
      ...
      {showResult && (
        <div className="card border-primary/20 bg-primary/5 animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex gap-2">
              <button className="p-2 hover:bg-white rounded-lg text-slate-400 hover:text-success transition-all"><ThumbsUp size={16} /></button>
              <button className="p-2 hover:bg-white rounded-lg text-slate-400 hover:text-danger transition-all"><ThumbsDown size={16} /></button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap gap-3">
              {['YÖK 2547 s.k. Md.44', 'Lisansüstü Yön. Md.12', 'Senato Kararı 2024/12'].map(ref => (
                <button key={ref} className="flex items-center gap-2 bg-white border border-gray-200 px-3 py-2 rounded-lg text-xs text-primary font-medium hover:border-primary transition-all">
                  {ref}
